 ...
 * 菜单操作model
 */
class DicDataModel extends BaseModel{
	...
	public function deleteData($map){
		...
		$data = $this
			...
			->select();
		if($count!=0){
			$log->addLog('DELETE','DicData',json_encode(array('ERROR::' => $emsg,'Data::'=>$data,'IP::'=>$ip,'Config::'=>$map)),'');
			return false;
		}
		$dd = $this->where(array($map))->select();
		$log->addLog('DELETE','DicData',json_encode($dd),'');
