 ...
 * 机构操作model
 */
class OrgnazationModel extends BaseModel{
	...
	public function deleteData($map){
		...
		$data = $this
			...
			->select();
		if($count!=0){
			$log->addLog('DELETE','Orgnazation',json_encode(array('ERROR::' => $emsg,'Data::'=>$data,'IP::'=>$ip,'Config::'=>$map)),'');
			return false;
		}
		$dd = $this->where(array($map))->select();
		$log->addLog('DELETE','Orgnazation',json_encode($dd),'');
	...
	public function getTreeData($type='tree',$order='',$p=1,$listRows=4){
			...
			{//数据过滤（超级管理员
				$condition['id'] = $_SESSION['user']['org_id'];
				//$condition['_logic'] = 'AND';
				//$condition["pid"] =  $_SESSION['user']['org_id'];
			}else
			{
				$condition["pid"] = "0";
			}
			...
			if(!empty($str))//数据过滤（超级管理员）
			{
				...
				if(empty($order)){
					...
					$data=$this->where($cond)->order('sortnum is null,'.$order)->select();
				}
			}
		// 获取树形或者结构数据
		if($type=='tree'){
			$data=\Org\Nx\Data::tree($data,'org_name','id','pid');
		}elseif($type="level"){
