...
class DepartmentModel extends BaseModel{
	...
	public function deleteData($map){
		...
		$data = $this
			...
			->select();
		if($count!=0){
			$log->addLog('DELETE','Department',json_encode(array('ERROR::' => $emsg,'Data::'=>$data,'IP::'=>$ip,'Config::'=>$map)),'');
			return false;
		}
	...
	public function getTreeData($type='tree',$order='',$p=1,$listRows=4){
			...
			if(empty($order)){
				...
				->alias('a')->join(' orgnazation b ON b.id= a.org_id')
				->where($cond)->select();
			}else{
				$data=$this->field('a.*,b.org_name')
				->alias('a')->join(' orgnazation b ON b.id= a.org_id')
		...
		// 获取树形或者结构数据
		if($type=='tree'){
			$data=\Org\Nx\Data::tree($data,'dept_name','id','pid');
		}elseif($type="level"){
