...
class EmployeeModel extends BaseModel{
	...
	public function deleteData($map){
		...
		$data = $this
			...
			->select();
		if($count!=0){
			$log->addLog('DELETE','Employee',json_encode(array('ERROR::' => $emsg,'Data::'=>$data,'IP::'=>$ip,'Config::'=>$map)),'');
			return false;
		}
		$dd = $this->where(array($map))->select();
		$log->addLog('DELETE','Employee',json_encode($dd),'');
	...
	public function getTreeData($type='tree',$order='',$p=1,$listRows=4){
			...
			if(empty($order)){
				...
				->alias('a')->join(' LEFT JOIN sys_orgnazation b ON b.id= a.org_id ')
				->where($cond)->select();
			}else{
				$data=$this->field('a.*,b.org_name')
				->alias('a')->join(' LEFT JOIN sys_orgnazation b ON b.id= a.org_id')
