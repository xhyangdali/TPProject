...
//******************权限***********************
    ...
    public function index($p = 1){
		...
		$ip = get_client_ip();
		$log = D("Log");
		$log->addLog('ACSESS','AuthRule',json_encode(array('Result::' => true,'Data::'=>'','IP::'=>$ip)),'');
        //$data=D('AuthRule')->getTreeData('tree','id','title');
		$totalRows = D('AuthRule')->getTreeDataCount('tree','id','title');
		$totalPages = 1;
		$listRows = 4;
		...
		$data=D('AuthRule')->getTreeData('tree','id','title','id','pid',$p,$listRows,1);
		$page =new Think\Page();
		$page->firstRow = 1;//
...
//*******************用户组**********************
    ...
    public function group($p = 1){
		...
		$ip = get_client_ip();
		$log = D("Log");
		$log->addLog('ACSESS','AuthGroup',json_encode(array('Result::' => true,'Data::'=>'','IP::'=>$ip)),'');
		$totalRows = D('AuthGroup')->count();
		$totalPages = 1;
		$listRows = 20;
		if($totalRows>$listRows)
		{
        ...
        $data=D('AuthGroup')->page($p,$listRows)->select();
		$page =new Think\Page();
		$page->firstRow = 1;//
		$page->listRows = $listRows;
		$page->totalRows = $totalRows;
    ...
    public function rule_group(){
        if(IS_POST){
            ...
            if ($result) {
				...
				$old = D('AuthGroup')->where($map)->select();
				if($data['rules'] == $old[0]["rules"])
				{
					$this->success('操作成功');
				}else
				{
...
//******************用户-用户组*******************
     ...
     * 管理员列表
     */
    public function admin_user_list($p=1){
		$ip = get_client_ip();
		$log = D("Log");
		$log->addLog('ACSESS','AuthGroupAccess',json_encode(array('Result::' => true,'Data::'=>'','IP::'=>$ip)),'');
		$dataA = D("AuthGroupAccess");
		$totalRows = $dataA->getAllDataCount();
		$totalPages = 1;
		$listRows = 20;
		if($totalRows>$listRows)
		{
        ...
        $data= $dataA->getAllPageData($p,$listRows);
        $page =new Think\Page();
		$page->firstRow = 1;//
		$page->listRows = $listRows;
		$page->totalRows = $totalRows;
