/* 
		...
		date:2017.10.26
		description:
			电子客票销售张数同比统计存储过程（查询统计）
			输入参数：station_fix_a(GQ公司客运站编码前缀) station_fix_b (XF公司客运站编码前缀) 
			channel_fix(客票渠道前缀) start_date(查询统计开始时间) end_date(查询统计截至时间)
			start_date_next(上一时段查询统计开始时间) end_date_next(上一时段查询统计截至时间)
			输出：分公司各客运站窗口售票金额集合
 */

DROP PROCEDURE IF EXISTS QDTB_ETICKET_CHARTS;

CREATE PROCEDURE QDTB_ETICKET_CHARTS(IN station_fix_a VARCHAR(20),IN station_fix_b VARCHAR(20),IN channel_fix VARCHAR(20),IN start_date DATETIME,IN end_date DATETIME,IN start_date_next DATETIME,IN end_date_next DATETIME)
BEGIN
		SELECT
			'Now' AS flag,/* 时段数标记 */
			sf.stationcode,/* 车站编码 */
...
CALL QDTB_ETICKET_CHARTS("10%","20%","20%","2017-10-28 00:00:00","2017-10-28 00:00:00","2017-10-27 00:00:00","2017-10-27 00:00:00");